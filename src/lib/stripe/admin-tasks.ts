...
export const upsertProductRecord = async (product: Stripe.Product) => {
    ...
    try {
        ...
        console.error(`upsertProductRecord Error: ${error}`);
...
export const createOrRetrieveCustomer = async ({email, uuid}:{email: string, uuid: string}) => {
    try {
        ...
        console.error(`createOrRetrieveCustomerError: ${error}`);
        const customerData: { metadata: { supabaseUUID: string }; email?: string } =
            ...
            try {
                ...
                throw new Error('Could not create or find the customer', stripeError)
            }
    }
}
...
export const copyBillingDetailsToCustomer = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
    ...
    try {
        ...
        throw new Error('Could not copy the customer\'s bulling details', error)
    }
}
...
export const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction=false) => {
    try {
        ...
        if (!customerData) {
            throw new Error('Could not find the customer')
        }
